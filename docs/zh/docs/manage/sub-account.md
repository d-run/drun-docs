...
主账号开启子账号功能后，主账号和子账号在费用中心中的可见范围如下：

| 功能 | 主账号 | 子账号 |
| --- | --- | --- |
| 钱包 | 可见，可管理主账号钱包 | 主账号未设置子账号限额时可见，可查看与主账号共享的余额 |
| 收支明细 | 可查看并按账号筛选主账号及子账号记录 | 仅可查看当前登录账号的记录 |
| 订单管理 | 可查看并按账号筛选主账号及子账号订单 | 仅可查看当前登录账号的订单 |
| 账单明细 | 可查看并按账号筛选主账号及子账号账单 | 仅可查看当前登录账号的账单 |
| 月账单 | 可查看月账单及按账号统计的详情 | 不可见 |
| 代金券 | 可见并管理主账号代金券 | 不可见 |

收支明细、订单列表和账单列表均展示用户名及 User ID。主账号可以通过 **账号** 下拉框选择主账号或子账号进行筛选。
...
主账号和子账号的关键操作会记录在操作审计中，便于追踪账号操作、资源变更和异常行为。

### 访问操作审计

1. 登录 d.run 平台。
2. 点击右上角头像，在下拉菜单中进入 **子账号**。
3. 在左侧导航栏点击 **操作审计**。

![子账号操作审计](images/sub-account-audit-zh.png)
...
- **账号**：选择需要查看的主账号或子账号。
- **产品模块**：按产品模块筛选操作记录，包括算力云、大模型服务平台、全局管理和费用中心。
- **发生时间**：按时间范围筛选审计记录。
...
| --- | --- |
| 发生时间 | 操作发生的时间。 |
| 资源类型 | 被操作资源的类型，例如 `Transaction`、`ModelServing`、`ContainerInstance` 和 `User`。 |
| 资源名称 | 被操作资源的名称或标识。 |
| 产品模块 | 操作所属的产品模块。 |
| 操作行为 | 执行的操作，例如 `Create`、`Login` 或 `Export`。 |
| 操作人 | 执行操作的账号。 |
| 来源 IP | 发起操作的来源 IP 地址。 |
| 状态 | 操作执行结果，例如成功或失败。 |

审计日志默认保留 365 天，超过保留期限的日志将自动删除。
