...
![Tensor Parallelism（张量并行）在 TP=4 时的数据流](./images/tp_vanilla_improved.svg)

<a id="figure1"></a>
<div style="text-align:center;">
图 1：Tensor Parallelism 在 TP=4 时的数据流
</div>
...
![Data Parallelism 在 DP=4 时的数据流](./images/data_parallelism.svg)

<a id="figure2"></a>
<div style="text-align:center;">
图 2：Data Parallelism 在 DP=4 时的数据流
</div>
...
![Vanilla Pipeline Parallelism 在 PP=4 时的数据流](./images/vanilla_pipeline_parallelism.svg)

<a id="figure3"></a>
<div style="text-align:center;">
图 3：Vanilla Pipeline Parallelism 在 PP=4 时的数据流
</div>

在[图 3](#figure3) 中，为了演示，单个请求在这种“原始”的流水线并行中被顺序处理。任意时刻只有一张 GPU 在工作，其余 GPU 处于空闲状态（流水线气泡）。每个请求的总延迟为 4 个顺序阶段，GPU 利用率仅为 25%（4 张 GPU 中只有 1 张在工作）。
...
如下方[图 4](#figure4) 所示，在 TP=8 且启用专家并行时，专家被分布到 8 张 GPU 上（每 GPU 32 个专家），并在每一层之后使用 AllReduce 通信。

![TP=8 且启用专家并行的单节点数据流](./images/tp_moe_EP_v2.svg)

<a id="figure4"></a>

<div style="text-align:center;">
图 4：TP=8 的单节点数据流（启用专家并行）
</div>

![TP=8 且未启用专家并行的单节点数据流](./images/tp_moe_v2.svg)

<a id="figure5"></a>

<div style="text-align:center;">
图 5：TP=8 的单节点数据流（未启用专家并行）
</div>
...
![DP=8 且未开启 Expert Parallelism 的数据流](./images/dp_without_ep_moe_v3.svg)
...
![DP=8 且开启 Expert Parallelism 的数据流](./images/dp_with_ep_moe_v2.svg)
...
![DeepSeek-R1 DP+EP vs TP+EP 扩展性能](./images/deepseek_r1_comparison.png)
...
![Qwen3-235B-A22B DP+EP vs TP+EP 扩展性能](./images/qwen3_final_comparison.png)
...
![Llama-4-Maverick-17B-128E 启用/不启用 EP 的 TP vs DP 扩展](./images/llama4_comparison.png)
