...
def get_sheet_content(
    app_token, table_id, view_id, tenant_access_token, page_size=100, page_token=None
):
...
def get_items(data):
    ...
    filtered_items = [ITEM_HEADER]
    for item in items:
        fields = item.get("fields", {})
        first_level = (
            ",".join([x.get("text", "") for x in fields.get("一级功能", [])])
            if fields.get("一级功能")
            else ""
        )
        second_level = (
            ",".join([x.get("text", "") for x in fields.get("二级功能", [])])
            if fields.get("二级功能")
            else ""
        )
        baseline = (
            ",".join([x.get("text", "") for x in fields.get("基线参数", [])])
            if fields.get("基线参数")
            else ""
        )
        version = ""
        ...
        if release_time and "value" in release_time and release_time["value"]:
            try:
                ...
                normal_date = datetime.fromtimestamp(timestamp, timezone.utc).strftime(
                    "%Y-%m-%d"
                )
...
def get_release_Dict(data):
    ...
    df_sorted = df.sort_values(
        by=["发版时间", "功能模块排序", "版本排序", "更新类型排序", "一级功能"],
    ...
    for _, row in df_sorted.iterrows():
        ...
        entry = {"二级功能": row["二级功能"], "基线参数": row["基线参数"]}
        result.setdefault(pub_date, OrderedDict()).setdefault(
            module, OrderedDict()
        ).setdefault(version, OrderedDict()).setdefault(update_type, []).append(
            (primary_func, entry)
        )
    # 组内一级功能排序
        ...
        for module in result[pub_date]:
            for version in result[pub_date][module]:
                for update_type in result[pub_date][module][version]:
                    result[pub_date][module][version][update_type].sort(
                        key=lambda x: x[0]
                    )
    return result
...
if __name__ == "__main__":
    ...
    original_data = get_sheet_content(app_token, table_id, view_id, tenant_access_token)
    if not original_data:
        logging.error("获取表格内容失败，程序退出")
        exit(1)
    get_release_info(original_data, filename=file_out_name)
