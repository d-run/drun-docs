...
为了降低使用门槛，d.run 采用 RBAC 模型将权限聚合成一个个角色，管理员只需要将角色授权给用户，该用户就一次性得到了该角色下聚合的一组权限。
...
参见上图，为了满足企业内各个部门的分支划分，d.run 引入了[层级](../user-guide/workspace/ws-folder.md)的概念，通常层级对应着不同的部门，每个层级可以包含一个或多个工作空间。

### 资源

[资源（Resource）](../user-guide/workspace/quota.md)泛指 d.run 平台上通过各个子模块创建的资源，是完成授权的具体数据。
